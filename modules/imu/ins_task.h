...
typedef struct
{
    float q[4];             // 四元数估计值必须在最前面，为了和 INS_t 内存对齐！
    float MotionAccel_b[3]; // 机体坐标加速度 (占位对齐)
    float MotionAccel_n[3]; // 绝对系加速度 (占位对齐)
    float AccelLPF;         // 加速度低通滤波系数 (占位对齐)
    float xn[3];            // (占位对齐)
    float yn[3];            // (占位对齐)
    float zn[3];            // (占位对齐)
...
// 为了避免强转错位，INS_t的设计必须与attitude_t前几项对应
typedef struct
{
    float q[4]; // 四元数估计值
