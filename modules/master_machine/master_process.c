...
HostInstance *HostInit(HostInstanceConf *host_conf)
{
    ...
    // 为上位机实例注册守护进程
    if (host_conf->daemon_conf.callback == NULL) {
        host_conf->daemon_conf.callback = HostOfflineCallback; // 默认离线回调函数
    }
    host_conf->daemon_conf.owner_id = _instance; // 将实例地址作为owner_id 
    host_conf->daemon_conf.init_count = 20; 
    _instance->daemon = DaemonRegister(&host_conf->daemon_conf);
...
uint8_t HostIsOnline(HostInstance *instance)
{
